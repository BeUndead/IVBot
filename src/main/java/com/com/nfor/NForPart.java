...
public class NForPart<T extends Number & Comparable<T>> {
    ...
    public final NForPart from(T... from) {
        if (this.n == 0) {
            ...
            throw new IllegalArgumentException(
                    String.format("from was the wrong length, expecting %d but received %d", n, from.length));
        }
    ...
    public final NForPart by(T... by) {
        if (this.n == 0) {
            ...
            throw new IllegalArgumentException(
                    String.format("by was the wrong length, expecting %d but received %d", n, by.length));
        }
    ...
    public final NFor to(T... to) {
        ...
        if (to.length != n) {
            throw new IllegalArgumentException(
                    String.format("to was the wrong length, expecting %d but received %d", n, to.length));
        }
    ...
    @SafeVarargs
    public final NFor<T> to(NForWhile.NForWhileTerm<T>... terms) {
        if (this.n == 0) {
            this.n = terms.length;
        } else if (this.n != terms.length) {
            throw new IllegalArgumentException(
                    String.format("terms was the wrong length, expecting %d but received %d", n, terms.length));
        }
        if (this.from == null) {
